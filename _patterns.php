...
<head>
	<title>模式入门 - Barebones 中文版</title>
	<meta charset="gb2312"/>
...
<?php
	...
	foreach ($files as $file):
		...
		echo '<summary title="显示标签和用法">&#8226;&#8226;&#8226;</summary>';
		echo '<section>';
		echo '<textarea rows="10" cols="30" class="input code">'.htmlspecialchars(file_get_contents($patterns_dir.'/'.$file)).'</textarea>';
		echo '<p class="caption"><strong>用法:</strong> '.htmlspecialchars(@file_get_contents($patterns_dir.'/'.str_replace('.html','.txt',$file))).'</p>';
		echo '</section>';
		echo '</details><!--/.primer-->';
